...
class register_model(nn.Module):
    def __init__(self, img_size=(160, 208, 160), mode='bilinear'):
...
def process_label():
    ...
    seg_table = [
                 1002, 1003, 1005, 1006, 1007, 1008, 1009, 1010, 1011, 1012, 1013, 1014, 1015, 1016, 1017, 1018, 1019,
                 1020, 1021, 1022, 1023, 1024, 1025, 1026, 1027, 1028, 1029, 1030, 1031, 1034, 1035, 2002, 2003, 2005,
                 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022,
                 2023, 2024, 2025, 2026, 2027, 2028, 2029, 2030, 2031, 2034, 2035,
                 3011, 3012, 3013, 3017, 3050, 3051, 3052, 3053, 3016, 3024,
                 ]
                 


    file1 = open('/home/hpe/RGY/SecondExperiment/Mind101/labels_DKT31.txt', 'r')
    ...
    for seg_label in seg_table:
        for line in Lines:
            line = re.sub(' +', ' ',line).split(' ')  #re.sub将line中的‘ +’替换为‘’
            try:
                int(line[0])
            except:
                continue
...
def dice_val_substruct(y_pred, y_true, std_idx):
    with torch.no_grad():
        y_pred = nn.functional.one_hot(y_pred, num_classes=64)
        y_pred = torch.squeeze(y_pred, 1)
        y_pred = y_pred.permute(0, 4, 1, 2, 3).contiguous()
        y_true = nn.functional.one_hot(y_true, num_classes=64)
        y_true = torch.squeeze(y_true, 1)
        y_true = y_true.permute(0, 4, 1, 2, 3).contiguous()
